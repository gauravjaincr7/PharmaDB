...
<body style="background-image:url(https://www.boswellpharmacy.com/wp-content/uploads/2017/01/ThinkstockPhotos-5433376822.jpg)">
	...
	<?php  
		...
		if(empty($_POST) or $_POST['Filter']=='' )    /*initial state or the state when the filter is empty*/
		{
			...
			$sql = "SELECT M.DRUG_ID,M.NAME AS NAME1,M.PRICE, M.QUANTITY, M.COMPANY_ID, P.Name,P.Address FROM medicine M
			INNER JOIN pharma_company P ON P.pharma_id=M.COMPANY_ID"; //You don't need a ; like you do in SQL
		...
		else
		{
			...
			$sql ="SELECT M.DRUG_ID,M.NAME AS NAME1,M.PRICE, M.QUANTITY, M.COMPANY_ID, P.Name,P.Address FROM medicine M
			INNER JOIN pharma_company P ON P.pharma_id=M.COMPANY_ID WHERE M.NAME = '$_POST[Filter]'" ;
	...
	?>
		...
		<table class="striped card">
			...
			echo "<tr><th>"."DRUG ID"."</th><th>"."NAME"."</th><th>"."PRICE"."</th><th>"."QUANTITY"."</th><th>"."COMPANY_ID"."</th></tr>";// tr- row , th - head cell , td - standard cell -->

	<?php
			while($row = mysqli_fetch_array($result))
			{
	?>
				<!-- echo "<tr><td>" . $row["DRUG_ID"] . "</td><td>" . $row["NAME"] . "</td><td>".$row["PRICE"] ."</td><td>".$row["QUANTITY"] ."</td><td>".$row["COMPANY_ID"] . "</td></tr>";  //$row['index'] the index here is a field name -->
				<tr>
